"""
Creates new satellite fields based on the layers from ToC.
...
def create_satellite_field(TOKEN, name, providers, start_time, geometry):
    ...
    More info at https://docs.withleaf.io/docs/crop_monitoring_endpoints#create-a-satellite-field.

    Parameters:
    - name (str): The field identification/fieldId
    - providers (List[str]): List of the providers. `sentinel` and `planet` are options available
    - start_time (str): Start time (ISO, "yyyy-mm-dd") meaning Leaf will return all images for your field since this date.
    - geometry (dict): The field multipolygon geometry in the GeoJSON format
    """
